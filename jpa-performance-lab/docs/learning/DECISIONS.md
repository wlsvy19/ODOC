...
- 결정: `인계 저장`을 비롯해 현재 진행 내용을 Git에 저장·푸시하라는 자연어 요청을 상태 문서 갱신, 한글 커밋과 `origin/main` 푸시의 명시적 승인으로 사용하고, `어디까지 진행했어?`를 재개 요청 문구로 사용한다.
...
## 2026-07-27 — Lombok 적용 범위

- 결정: 입력 DTO는 Lombok `@Data`, JPA 엔티티는 `@Getter`를 사용하고 엔티티 상태 변경은 생성·수정 행위 메서드로 제한한다.
- 이유: 반복적인 접근자 코드는 줄이면서 엔티티의 임의 setter 호출은 막기 위해서다.
- 변경 조건: 불변 DTO나 Java record로 입력 모델을 전환하면 Lombok 적용 범위를 다시 정한다.

## 2026-07-27 — P6Spy 학습용 SQL 로그
