...
const getReportRequestData = computed(() => {
  return createOzDataset('/SALES/dailyExemptDetail.ozr', reportParams.value);
});

const handleSearch = async () => {
  isLoading.value = true;
  try {
    const response = await request('post', 'api/sales/getDailyExemptDetail', {
      IC_CODE: authStore.getIcCode,
