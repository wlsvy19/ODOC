...
const headers = ref([
  {
    title: '구분',
    children: [{ title: '차로명', key: 'LANE_NM', width: '150' }],
  },
  {
    title: '차종구분',
    children: [
...
const trafficLineData = ref([]);
const ozSearchData = ref([]);

const handleSearch = async () => {
  try {
    isLoading.value = true;
    const data = await request('post', 'api/traffic/getTrafficLine', {
      ...searchData.value,
      ...{
...
const handleExcel = () => {
  const row = 'mix';
  if (trafficLineData.value.length === 0) {
    alert(`조회된 데이터가 없습니다.`);
    return;
  }
  ...
  if (confirm(`엑셀 다운로드를 진행하시겠습니까?`)) {
    ...
    excelDownload(row, searchHeader.value, searchData.value, excelHeaders, trafficLineData.value, '차로별 교통량 조회', '차로별 교통량 조회');
  } else {
    alert(`엑셀다운로드 취소`);
  }
};
const handlePrint = async () => {
  try {
    ...
    jsonData.value = createOzDataset('/TRAFFIC/trafficLine.ozr', {
      CSV_DATA: trafficLineData.value,
    ...
    alert(`차로별 교통량 조회 Report Error가 발생했습니다.\n문제가 지속된다면 관리자한테 문의하세요.`);
  } finally {
    isLoading.value = false;
  }
};
