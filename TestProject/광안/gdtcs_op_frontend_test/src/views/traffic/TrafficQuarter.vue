...
const searchHeader = ref([{ label: '조회기간', key: 'START_DATE', type: 'date', dateType: 'year' }]);
...
const mainHeaders = ref([
  { title: '년별', key: 'YEAR_DP', width: '100', excelWidth: 30 },
  { title: '분기구분', key: 'QUARTER_DP', width: '90', excelWidth: 30 },
  {
    title: '요금소',
    align: 'center',
    children: [
      { title: '소계', key: 'SUB_TOT_CNT', width: '120', customBodyCellStyle: 'table-body-style-right' },
...
const handleSearch = async () => {
  try {
    ...
    const response = await request('post', 'api/traffic/getTrafficQuarter', {
      WORK_YEAR: searchData.value['START_DATE'],
...
const handlePrint = () => {
  if (mainContents.value.length === 0) {
    ...
    reportRequestData.value = createOzDataset('/TRAFFIC/trafficQuarter.ozr', {
