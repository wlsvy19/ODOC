<template>
  ...
  <TableComponent
    :heightOffset="285"
    rowType="mix"
    scrollKey="trafficDailyDirectionData"
    :custom-body-row-style="setCustomBodyRowStyleSummary"
    :headers="headers"
    :contents="trafficDailyDirectionData"
  />
...
const setCustomBodyRowStyleSummary = (item) => {
  if (item.TRF_DATE === '합계' || item.TRF_DATE === '합계(%)') {
    return 'table-body-style-summary-1';
  } else if (item.TRF_DATE === '소계') {
...
const trafficDailyDirectionData = ref([]);
const ozSearchData = ref([]);

const handleSearch = async () => {
  try {
    isLoading.value = true;
    const data = await request('post', 'api/traffic/getTrafficDailyDirection', {
      ...searchData.value,
      ...{
    ...
    trafficDailyDirectionData.value = data;
    ...
    alert(`일별/방향별 교통량 조회 중 에러가 발생했습니다.\n문제가 지속된다면 관리자한테 문의하세요.`);
  } finally {
...
const handleExcel = () => {
  const row = 'mix';
  if (trafficDailyDirectionData.value.length === 0) {
    alert(`조회된 데이터가 없습니다.`);
    return;
  }
  ...
  if (confirm(`엑셀 다운로드를 진행하시겠습니까?`)) {
    ...
    excelDownload(
      ...
      searchData.value,
      excelHeaders,
      trafficDailyDirectionData.value,
      '일별방향별 교통량 조회',
      '일별방향별 교통량 조회',
    );
  } else {
...
const handlePrint = async () => {
  try {
    ...
    jsonData.value = createOzDataset('/TRAFFIC/trafficDailyDirection.ozr', {
      CSV_DATA: trafficDailyDirectionData.value,
      START_DATE: searchData.value['START_DATE'],
      END_DATE: searchData.value['END_DATE'],
      TITLE_NM: '일별_방향별 교통량 조회',
      IC_CODE: authStore.getIcCode,
    ...
    alert(`일별/방향별 교통량 조회 Report Error가 발생했습니다.\n문제가 지속된다면 관리자한테 문의하세요.`);
  } finally {
    isLoading.value = false;
  }
};
