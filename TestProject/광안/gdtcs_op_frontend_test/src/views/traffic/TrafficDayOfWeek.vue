...
const mainHeaders = ref([
  {
    title: '구분',
    align: 'center',
    children: [{ title: '요일명', key: 'WORK_DAY_DP', width: '100' }],
  },
  {
    title: '차종구분',
    align: 'center',
    children: [
      { title: '경형', key: 'CAR1_CNT', width: '100', customBodyCellStyle: 'table-body-style-right' },
      { title: '소형', key: 'CAR2_CNT', width: '100', customBodyCellStyle: 'table-body-style-right' },
      { title: '중형', key: 'CAR3_CNT', width: '100', customBodyCellStyle: 'table-body-style-right' },
      { title: '대형', key: 'CAR4_CNT', width: '100', customBodyCellStyle: 'table-body-style-right' },
      { title: '합계', key: 'CAR_TOT_CNT', width: '100', customBodyCellStyle: 'table-body-style-right' },
    ],
  },
  {
    title: '징수구분',
    align: 'center',
    children: [
      { title: '하이패스선불', key: 'PAY1_CNT', width: '100', customBodyCellStyle: 'table-body-style-right' },
      { title: '하이패스후불', key: 'PAY2_CNT', width: '100', customBodyCellStyle: 'table-body-style-right' },
      { title: '면제', key: 'PAY3_CNT', width: '100', customBodyCellStyle: 'table-body-style-right' },
      { title: '위반', key: 'PAY4_CNT', width: '100', customBodyCellStyle: 'table-body-style-right' },
      { title: '사전등록', key: 'PAY5_CNT', width: '100', customBodyCellStyle: 'table-body-style-right' },
    ],
  },
]);

const setCustomBodyRowStyleSummary = (item) => {
  if (item.WORK_DAY_DP === '합계') {
...
const handleSearch = async () => {
  try {
    ...
    const data = await request('post', 'api/traffic/getTrafficDayOfWeek', {
...
const handlePrint = () => {
  if (mainContents.value.length === 0) {
    ...
    reportRequestData.value = createOzDataset('/TRAFFIC/trafficDayOfWeek.ozr', {
