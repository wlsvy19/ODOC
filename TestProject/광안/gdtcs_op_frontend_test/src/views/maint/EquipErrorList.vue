<template>
  ...
  <TableComponent
    ...
    :contents="equipErrorListData"
    rowType="1"
    scrollKey="equipErrorListData"
  />
...
const headers = ref([
  ...
  { title: '발생일시', key: 'DATE_DT', width: '200' },
  { title: '기종', key: 'KIND', width: '150' },
  { title: '이상내역', key: 'DETAIL', width: '200' },
  { title: '발생/복구구분', key: 'ORGTYPE', width: '150' },
  { title: '근무상태', key: 'WORKTYPE', width: '100' },
...
const equipErrorListData = ref([]);
const ozSearchData = ref([]);

const handleSearch = async () => {
  try {
    isLoading.value = true;
    const data = await request('post', 'api/maint/getEquipErrorList', {
      ...searchData.value,
      ...{
...
const handleExcel = () => {
  const row = 1;
  if (equipErrorListData.value.length === 0) {
    alert(`조회된 데이터가 없습니다.`);
    return;
  }
  ...
  if (confirm(`엑셀 다운로드를 진행하시겠습니까?`)) {
    ...
    excelDownload(row, searchHeader.value, searchData.value, excelHeaders, equipErrorListData.value, '장비이상내역조회', '장비이상내역조회');
  } else {
    alert(`액셀다운로드 취소`);
  }
};
const handlePrint = async () => {
  try {
    ...
    jsonData.value = createOzDataset('/MAINT/equipErrorList.ozr', {
      CSV_DATA: equipErrorListData.value,
    ...
    alert(`장비이상내역조회 Report Error가 발생했습니다.\n문제가 지속된다면 관리자한테 문의하세요.`);
  } finally {
    isLoading.value = false;
  }
};
