...
const getJsonData = computed(() => {
  jsonData.value = createOzDataset('/DAY/dayFin.ozr', ozParam.value);
...
const handleSearch = async () => {
  ...
  const result = await request('post', 'api/day/getDayFinReport', {
  ...
  if (isEmpty(result?.day)) {
    showMessage(`${dayjs(searchData.value.WORK_DATE).format('YYYY년 MM월 DD일')} 일마감이 완료되지 않았습니다.`, 'error', 5000);
