...
const getJsonData = computed(() => {
  jsonData.value = createOzDataset('/DAY/dayFinPeriodNew.ozr', ozParam.value);
  ...
  if (!data || data.length === 0) {
    showMessage(
      `${dayjs(searchData.value.START_DATE).format('YYYY년 MM월 DD일')} ~ ${dayjs(searchData.value.END_DATE).format(
        'YYYY년 MM월 DD일',
      )} 사전등록/공차택시 반영 재집계 처리가 완료되지 않았습니다.`,
      'error',
      5000,
    );
...
const handleSearch = async () => {
  ...
  const result = await request('post', 'api/day/getDayFinPeriodReportNew', {
