...
class AppContext {
    ...
    private void doResource() {
        // map에 저장된 객체의 instance value 중에 @Resource가 붙어 있으면
        // map에서 instance value의 이름에 맞는 객체를 찾아서 연결(객체의 주소를 instance value저장)
        try {
    ...
    private void doAutowired() {
        // map에 저장된 객체의 instance value 중에 @Autowired가 붙어 있으면
        // map에서 instance value의 타입에 맞는 객체를 찾아서 연결(객체의 주소를 instance value저장)
        try {
            for (Object bean : map.values()) {
                for (Field fld : bean.getClass().getDeclaredFields()) {
    ...
    private void doComponentScan() {
        try {
            ...
            Set<ClassPath.ClassInfo> set = classPath.getTopLevelClasses("com.fastcampus.ch3.diCopy4"); // 패키지이름 적기
...
public class Main4 {
    public static void main(String[] args) throws Exception {
        ...
        // 수동으로 객체에 연결 -> Autowired와 Resource를 쓰면 자동으로 연결 됨
        // car.engine = engine;
        // car.door = door;
